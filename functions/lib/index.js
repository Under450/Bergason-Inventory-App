...
exports.dailyReminderCheck = exports.sendInventoryEmail = void 0;
...
// Helpers
// ─────────────────────────────────────────────────────────────────────────────
const generateReference = (propertyId) => {
    const now = new Date();
...
const formatTimestamp = (d) => d.toLocaleString('en-GB', {
...
const downloadPDF = async (path) => {
    if (!path)
        return null;
    try {
        const [buf] = await (0, storage_1.getStorage)().bucket().file(path).download();
        return buf;
    }
    catch (_a) {
        return null;
...
const sendConfirmationToOffice = async (transporter, opts) => {
...
exports.sendInventoryEmail = functionsV1
    .region('europe-west2')
    ...
    const dispatchRecord = {
        ...
        sentAt: firestore_1.FieldValue.serverTimestamp(), sentAtISO: sentAt.toISOString(),
    ...
    await db.collection('inventories').doc(d.firestoreToken)
        .update(Object.assign({}, firestoreUpdate));
    res.json({ success: true, reference });
});
// ─────────────────────────────────────────────────────────────────────────────
// Scheduled: daily reminder + expiry check (runs every day at 09:00 London)
// ─────────────────────────────────────────────────────────────────────────────
exports.dailyReminderCheck = (0, scheduler_1.onSchedule)({ schedule: '0 9 * * *', timeZone: 'Europe/London', secrets: [ionosPassword], region: 'europe-west2' }, async () => {
    const db = (0, firestore_1.getFirestore)();
    ...
    for (const docSnap of snapshot.docs) {
        ...
        const reviewSentAt = data.reviewSentAt || 0;
...
async function sendReminderEmail(transporter, data, token, day, db) {
...
async function sendExpiryProofEmail(transporter, data, token, db) {
    const reference = generateReference();
    ...
    const auditRows = dispatches.docs.map(d => {
        const dd = d.data();
        const ts = dd.sentAt instanceof firestore_1.Timestamp ? dd.sentAt.toDate() : new Date(dd.sentAtISO || 0);
